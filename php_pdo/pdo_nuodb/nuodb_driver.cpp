...
//static int nuodb_alloc_prepare_stmt(pdo_dbh_t*, const char*, long, XSQLDA*, isc_stmt_handle*, HashTable* TSRMLS_DC);
...
// TODO: do entire function
	...
	do {
		...
		if (!nuodb_alloc_prepare_stmt(dbh, sql, sql_len, &num_sqlda, &s, np TSRMLS_CC)) {
			break;
		}
...
// TODO: do entire function
	...
	/* allocate and prepare statement */
	if (!nuodb_alloc_prepare_stmt(dbh, sql, sql_len, &out_sqlda, &stmt, 0 TSRMLS_CC)) {
		return -1;
	}
...
static void nuodb_info_cb(void *arg, char const *s) /* {{{ */
{
	if (arg) {
	...
	switch (attr) {
		...
		case PDO_ATTR_SERVER_INFO:
			*tmp = 0;
			...
			// TODO: check version
			/*
			if (!isc_version(&H->db, nuodb_info_cb, (void*)tmp)) {
				ZVAL_STRING(val,tmp,1);
				return 1;
			}
			*/
...
static int pdo_nuodb_handle_factory(pdo_dbh_t *dbh, zval *driver_options TSRMLS_DC) /* {{{ */
{
	...
	pdo_nuodb_db_handle *H = dbh->driver_data = pecalloc(1,sizeof(*H),dbh->is_persistent);
